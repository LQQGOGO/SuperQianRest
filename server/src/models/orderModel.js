...
class Order {
  static async getAll(options = {}) {
    try {
      ...
      if (options.search) {
        query += ' AND (o.order_number LIKE ? OR u.username LIKE ? OR u.name LIKE ? OR o.phone LIKE ?)';
      ...
      const [countResult] = await db.query(
        `SELECT COUNT(*) as total FROM orders o 
         LEFT JOIN users u ON o.user_id = u.id
         WHERE 1=1 ${
           options.status ? ' AND o.status = ?' : ''
         } ${
           options.startDate ? ' AND DATE(o.created_at) >= ?' : ''
         } ${
           options.endDate ? ' AND DATE(o.created_at) <= ?' : ''
         } ${
           options.search ? ' AND (o.order_number LIKE ? OR u.username LIKE ? OR u.name LIKE ? OR o.phone LIKE ?)' : ''
         }`,
        params
      );
